        ...
        if let Token::Char(a) = curr_token {
            if a == "let" {
                ...
                if let Token::Char(b) = curr_token {
                    ...
                    return Err(format!("Parser Variable: Expected char got: {:?} at {i}", curr_token).into());
                }
            ...
            if a == "if" {
                ...
                let result = parser(i, tokens.to_vec())?;
                    ast.push(Node::IfStatement { condition, body: result.node });
                i = result.end_num + 1;
            ...
            if a == "while" {
                ...
                // Creat the body of the statement
                let result = parser(i, tokens.to_vec())?;
                    ast.push(Node::WhileLoop { condition, body: result.node});
                i = result.end_num + 1;
            ...
            if a == "for" {
                ...
                if let Token::Char(c) = curr_token {
                    ...
                    return Err(format!("Parser For loop: Expected Char got: {:?} at {i}", curr_token).into());
                }
                ...
                match curr_token {
                    ...
                    _ => {
                        return Err(
                            format!("Parser For loop: Expected Num/Char got: {:?} at {i}", curr_token).into()
                        )
                    }
                }
            ...
            if a == "fn" {
                ...
                if let Token::Char(c) = curr_token {
                    ...
                    return Err(format!("Parser Function: Expected Char got: {:?} at {i}", curr_token).into());
                }
                ...
                // Creat the body of the statement
                let result = parser(i, tokens.to_vec())?;
                ast.push(Node::Function { name, input, body: result.node});
                i = result.end_num + 2;
            ...
            if let Token::BracketOpen(Bracket::Paren) = tokens[i + 1] {
                ...
                match curr_token {
                    ...
                    _ => return Err(format!("Parser Function call: Expected Char got: {:?} at {i}", curr_token).into()),
                }
...
pub fn code_generator(ast: Vec<Node>) -> Result<String, Box<dyn Error>> {
    ...
    for i in ast {
        match i {
            ...
            Node::Veriable { name, value } => program.push_str(format!("let {}{}", name ,code_generator(value)?).as_str()),
            Node::VeriableCall(c) => program.push_str(format!("{c}").as_str()),
            Node::IfStatement { condition, body } => program.push_str(format!("if ({}){{\n{}}}\n", code_generator(condition)?, code_generator(body)?).as_str()),
            Node::ElseStatement {body} => program.push_str(format!("else{{\n{}}}\n", code_generator(body)?).as_str()),
            Node::WhileLoop { condition, body }=> program.push_str(format!("while ({}){{\n{}}}\n", code_generator(condition)?, code_generator(body)?).as_str()),
            ...
            _ => return Err(format!("Code Generator: Unexpected Node: {:?}", i).into()),
        }
    }
    Ok(program)
}
